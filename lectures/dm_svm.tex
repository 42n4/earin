...
\addlegendentry{$f(a)+\frac{1}{2}\times g(a)$}
\addplot[color=green,mark=triangle] {x^2-3*x+2}; 
\addlegendentry{$f(a)+\lambda\times g(a)$}
\addplot[color=black,mark=o] {x^2+2}; 
\addlegendentry{$f(a)+1\times g(a)$}
\end{axis} 
\end{tikzpicture} 
\begin{block}{}
\begin{itemize}
\item $\lambda = 1$ for Lagrangian $\max_{\lambda>0}\min_{a}f(a)+\lambda\times g(a)$ dual optimal solution.
